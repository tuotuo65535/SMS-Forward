    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Telephony.Sms.Intents.SMS_RECEIVED_ACTION.equals(intent.getAction())) {
            for (SmsMessage smsMessage : Telephony.Sms.Intents.getMessagesFromIntent(intent)) {
                ...
                String filter = context.getSharedPreferences("data", Context.MODE_PRIVATE).getString("filter", "");
                if(filter == null || filter.equals("") || messageBody.indexOf(filter) < 0) {
                    return;
                }
                ...
                String message = messageBody.substring(messageBody.indexOf("密码"));
